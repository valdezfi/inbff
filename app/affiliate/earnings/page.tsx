...
export default async function AffiliateEarningsPage() {
  ...
  const summaryCards = [
    ...
    { label: "Total",    value: `$${(totalPending + totalPaid).toFixed(2)}`, count: allCommissions.length, icon: DollarSign, iconColor: "text-[#006cd2]", iconBg: "bg-[#d0e9f7]", gradClass: "stat-indigo" },
  ];
  ...
  return (
    <div className="space-y-7 stagger">
      ...
      <div className="grid grid-cols-3 gap-4">
        {summaryCards.map((s) => (
          <div key={s.label} className={`rounded-none border border-slate-200/80 p-6 card-shadow ${s.gradClass}`}>
            <div className="flex items-center justify-between mb-4">
              <div className={`flex h-10 w-10 items-center justify-center rounded-none ${s.iconBg}`}>
                <s.icon className={`h-5 w-5 ${s.iconColor}`} />
              </div>
              <span className={`text-[10px] font-bold uppercase tracking-wider rounded-none px-2 py-0.5 ${s.iconBg} ${s.iconColor}`}>
                {s.count} total
              </span>
            </div>
      ...
      <div className="rounded-none border border-slate-200/80 bg-white overflow-hidden card-shadow">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-none bg-[#e8f4fb]">
              <TrendingUp className="h-4 w-4 text-[#006cd2]" />
            </div>
        ...
        {allCommissions.length === 0 ? (
          <div className="px-6 py-14 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-none bg-slate-100 mx-auto mb-4">
                ...
                return (
                  <div key={c.id} className="grid grid-cols-4 gap-4 px-6 py-4 items-center hover:bg-slate-50/80 transition-colors">
                    <div className="col-span-2 flex items-center gap-3">
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-none bg-gradient-to-br ${colors[colorIdx]} text-xs font-bold text-white shadow-sm`}>
                        {initial}
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-slate-800">{c.programName}</p>
                        <p className="text-xs text-slate-400">
                          {new Date(c.createdAt).toLocaleDateString()} · <span className="text-[#006cd2]">{c.rate}%</span> rate
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      {c.status === "paid" ? (
                        ...
                        <span className="inline-flex items-center gap-1 rounded-none bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700">
