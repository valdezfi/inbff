...
import {
  ...
  Menu, X, LogOut, Bell, Store, ChevronRight, Settings,
} from "lucide-react";
...
export default function AffiliateShell({ children, userName, userEmail, role }: {
  children: React.ReactNode; userName: string; userEmail: string; role: string;
}) {
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <aside className={`sidebar-gradient-affiliate flex flex-col ${mobile ? "w-64" : "w-60 hidden lg:flex"}`}>
      {/* Logo */}
      ...
      <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto mt-2">
        ...
        {nav.map(({ label, href, icon, exact }) => (
          <NavItem key={href} label={label} href={href} icon={icon} exact={exact}
            pathname={pathname} onClick={() => setOpen(false)} />
        ))}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-[#e4e8ed] bg-white px-5 lg:px-8">
          <div className="flex items-center gap-3">
            <button className="lg:hidden text-slate-500 hover:text-slate-800 p-1.5 hover:bg-[#f0f4f8] transition-colors" onClick={() => setOpen(true)}>
              <Menu className="h-5 w-5" />
            </button>
            <div className="hidden lg:flex items-center gap-2 text-sm">
              <span className="text-[#6b7378] font-medium">Affiliate</span>
