    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center stagger">
        <div className="relative mb-8">
          ...
          <span className="absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-none bg-amber-400 shadow-md">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-none bg-gradient-to-r bg-[#006cd2] px-6 py-3 text-sm font-semibold text-white shadow-lg  hover: hover:opacity-90 transition-all"
        >
  ...
  return (
    <div className="space-y-8 stagger">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {stats.map((s) => {
          const inner = (
            <div className={`relative overflow-hidden rounded-none border border-slate-200/80 p-5 card-shadow  transition-all group ${s.gradientClass}`}>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-none border border-slate-200/80 bg-white p-6 card-shadow">
          ...
          <Link
            ...
            className="mt-5 flex items-center justify-between rounded-none bg-slate-50 px-4 py-2.5 text-xs font-medium text-slate-600 hover:bg-[#e8f2fb] hover:text-[#0053a3] transition-colors"
          >
            Manage payouts
        ...
        <div className="rounded-none border border-slate-200/80 bg-white p-6 card-shadow">
          <div className="flex items-center gap-2 mb-5">
            <div className="flex h-8 w-8 items-center justify-center rounded-none bg-[#e8f2fb]">
              <Zap className="h-4 w-4 text-[#006cd2]" />
            </div>
            <h3 className="text-sm font-semibold text-slate-700">Quick Actions</h3>
          </div>
          <div className="space-y-2">
            {[
              { label: "Create new program", href: "/dashboard/programs/new",       color: "hover:bg-[#e8f2fb] hover:text-[#0053a3]" },
              { label: "Connect a store",    href: "/dashboard/connect-shopify",    color: "hover:bg-[#e8f2fb] hover:text-violet-700" },
              { label: "View payouts",       href: "/dashboard/payouts",            color: "hover:bg-emerald-50 hover:text-emerald-700" },
              { label: "Browse marketplace", href: "/marketplace",                  color: "hover:bg-[#e8f4fb] hover:text-[#0053a3]" },
            ].map((a) => (
              <Link
                key={a.href}
                href={a.href}
                className={`flex items-center justify-between rounded-none bg-slate-50 px-4 py-2.5 text-xs font-medium text-slate-600 transition-colors ${a.color}`}
              >
                {a.label}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link
            href="/dashboard/programs"
            className="text-xs font-medium text-[#006cd2] hover:text-[#0053a3] flex items-center gap-1 transition-colors"
          >
        ...
        {programs.length === 0 ? (
          <div className="rounded-none border border-dashed border-slate-300 bg-slate-50 p-12 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-none bg-[#e8f2fb] mx-auto mb-4">
              <Users className="h-7 w-7 text-[#006cd2]" />
            </div>
            ...
            <Link
              href="/dashboard/programs/new"
              className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#006cd2] hover:text-[#0053a3]"
            >
              Create your first program <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        ) : (
          <div className="rounded-none border border-slate-200/80 bg-white overflow-hidden card-shadow divide-y divide-slate-100">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-none bg-gradient-to-br bg-[#006cd2] shadow-md">
                      <Users className="h-5 w-5 text-white" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-slate-900 group-hover:text-[#006cd2] transition-colors">
                          {p.name}
                        </p>
                        <span className={`inline-flex items-center rounded-none px-2 py-0.5 text-[10px] font-semibold ${statusColor[p.status] ?? "badge-draft"}`}>
                          {p.status}
                        </span>
                      </div>
